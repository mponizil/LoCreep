...
def upload_creep_photo(request):
    ...
    if request.method == 'POST':
        ...
        if f is None:
            return render(request, 'error.html', { 'error': 'Please choose a file to upload.' })
        
        try:
            tmp = Image.open(f)
            tmp.verify()
        except IOError:
            return render(request, 'error.html', { 'error': 'Invalid image file. Please try another.' })
        ...
        try:
            ...
            return render(request, "error.html", { 'error': 'Invalid creep.' })
