...
def valid_email(request, rand_str):
    try:
        ...
        return render(request, "error.html", { 'error': "Invalid registration key." })
    ...
    return render(request, "valid-email.html")
...
def send_again(request):
    ...
    try:
        registration_key = RegistrationKey.objects.get(user=request.user)
        rand_str = registration_key.rand_str
    except RegistrationKey.DoesNotExist:
    ...
    server.sendmail(from_addr, request.user.email, msg.as_string())
    server.quit()
    # email sent
...
def welcome_pg(request):
    if not request.user.is_authenticated():
        return render(request, "welcome.html", { 'header': 'large' })
    else:
...
def login_pg(request):
    return render(request, "login.html", { 'header': 'large' })
